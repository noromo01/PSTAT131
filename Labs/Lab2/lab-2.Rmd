...
#splits the diamonds dataset into two categories, 80% is in one cat and 20% is in another
diamonds_split <- initial_split(diamonds, prop = 0.80,
                                strata = price)

#training is where 80% of the data is going, 20% is where the rest of the data is going
...
```{r}
#like the lm function, price is being regressed on all other predictors
...
```{r}
#step_dummy is used to dummy code all categorical predictors
...
```{r}
#just displays the head of the dataset, note that kable, kable_styling, scroll_box are used for formatting while bake and prep are used to read in the data
...
```{r}
#set up the workflow, use the diamonds_recipe format, diamonds_recipe is a recipe datatype
lm_wflow <- workflow() %>% 
  add_model(lm_model) %>% 
...
```{r}
#fit the model to the diamonds training data
...
```{r}
airquality %>% 
  summarise(mean(Ozone, na.rm = TRUE))
```
...
Another way is to remove all observations with missing data from the dataset, which is what `geom_histogram()` and `mean(na.rm=TRUE)` do by default. We could do that before we split the data like so:

```{r}
#drop all NAs observations from the dataset
