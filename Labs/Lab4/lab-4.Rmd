...
NOTE from NOAH: Validation set creates a set for you to test different models on. The testing set is reserved for a final testing round.
...
Auto_recipe_SLR <- recipe(mpg~horsepower, data=Auto_train)

lm_workflow_SLR <- workflow() %>% 
  add_model(lm_mod) %>% 
  add_recipe(Auto_recipe_SLR)

lm_fit_val_SLR <- lm_workflow_SLR %>% 
  fit_resamples(resamples=Auto_valid)

collect_metrics(lm_fit_val_SLR)
...
knn_wkflow_SLR <- workflow() %>% 
  add_model(knn_mod) %>% 
  add_recipe(Auto_recipe_SLR)

knn_fit_val_SLR <- knn_wkflow_SLR %>% 
  fit_resamples(resamples=Auto_valid)

collect_metrics(knn_fit_val_SLR)
